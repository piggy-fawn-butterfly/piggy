...
export class res {
  ...
  load(paths, onprogress = function() {}, oncomplete = function() {}) {
    ...
    for (let path of paths) {
      ...
      if (!cls) continue;
      if (!cc[cls]) continue;
      if (cc.js.getClassName(cc[cls]) === "cc.Asset" || cc.js.getClassName(cc.js.getSuper(cc[cls])) === "cc.Asset") valid_paths.set(path, cc[cls]);
    }
  ...
  _linkPathAndType() {
    ...
    for (let path in assets) {
      ...
      _entries instanceof Array ? entries.push(..._entries) : entries.push(_entries);
      let types = [];
    ...
    piggy.logger.info(piggy.i18n.t(piggy.i18nK.resources_type_in_cache), ...Array.from(_types));
  }
  ...
  _getCCLoaderCacheCount() {
    ...
    for (let url in cache) {
      if (cache.hasOwnProperty(url)) {
        ...
        piggy.constants.RES_TYPE_IN_CACHE_TO_EXCLUDES.has(res_type) ? data.excludes++ : data.refers++;
      }
    }
    return data;
  }
