(function() {
  ...
  var devices = [
    { name: "iPhone 5", width: 320, height: 568, ratio: 2 },
    { name: "iPhone 6/7", width: 375, height: 667, ratio: 2 },
    { name: "iPhone 6/7 Plus", width: 414, height: 736, ratio: 3 },
    { name: "iPhone X", width: 375, height: 812, ratio: 3 },

    { name: "iPad / Air 2", width: 768, height: 1024, ratio: 2 },
    { name: "iPad Pro 10.5-inch", width: 417, height: 556, ratio: 2 },
    { name: "iPad Pro 12.9-inch", width: 512, height: 683, ratio: 2 },

    { name: "Huawei P9", width: 540, height: 960, ratio: 2 },
    { name: "Huawei Mate9/P20 Pro", width: 720, height: 1280, ratio: 2 },

    { name: "Nexus 5", width: 360, height: 640, ratio: 3 },
    { name: "Nexus 6", width: 412, height: 732, ratio: 3.5 },
    { name: "Nexus 7", width: 960, height: 600, ratio: 2 }
  ];
  ...
  var isLandscape = designWidth > designHeight;
  ...
  let _scale = target_width / designWidth;
  let devices_after = [];
  let ratio_devices = [];
  let _idx = 0;
  devices.forEach(function(info) {
    ...
    let ratio = designWidth / info.width;
    info.width = info.width * ratio * _scale;
    info.height = Math.ceil(info.height * ratio * _scale);
    let _ratio = Math.ceil((info.width / info.height) * 100) * 0.01;
    let isOK = true;
    for (let i = 0; i < ratio_devices.length; i++) {
      if (Math.abs(Math.floor(100 * (_ratio - ratio_devices[i]))) === 0) {
        isOK = false;
        break;
      }
    }
    if (isOK) {
      ...
      devices_after.push(d);
      opt.text = d.name + ` [${d.width}x${d.height}]-${d.ratio}`;
  ...
  function getEmulatedScreenSize() {
    ...
    if (idx === "0") {
      ...
      var info = devices_after[parseInt(idx) - 1];
      w = info.width;
      h = info.height;
    }
    return {
  ...
  function onload() {
    ...
    cc.game.run(option, function() {
      ...
      cc.loader.load("preview-scene.json", function(error, json) {
        ...
        cc.AssetLibrary.loadJson(json, function(err, sceneAsset) {
          if (err) {
            cc.error(err.stack);
            return;
          }
